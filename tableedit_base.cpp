...
void tableedit_base::on_pushButton_4_clicked()
{
    ...
    if(db.open())
    {
        ...
        if(query.exec("SELECT * FROM [Uczestnik]"))
        {
            int i=0;
             while(query.next())
             {
                 setValue(i, query.value(1).toString(), query.value(2).toString(), query.value(3).toString(), query.value(4).toString(), query.value(5).toString());
                 i++;
             }
        }
        db.close();
